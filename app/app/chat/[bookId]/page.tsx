...
export default function ChatDetailPage() {
  ...
  const handleSendMessage = useCallback(async (content?: string) => {
    ...
    try {
      ...
      const rem = Number.isNaN(nextRem) ? 0 : nextRem;
      if (rem === 0 && response.plan === 'free') {
          ...
          <div className="shrink-0 border-t border-border bg-background/50 px-4 py-3">
            {isBlocked && (
              <div className="max-w-[720px] mx-auto mb-2 text-center">
                <p className="text-xs text-danger">You&apos;ve used all free messages today. <button onClick={() => setShowPaywall(true)} className="text-primary underline cursor-pointer">Upgrade for unlimited messages.</button></p>
              </div>
            )}
