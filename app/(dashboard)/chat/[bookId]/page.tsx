...
const VoiceChat = dynamic(
	() => import('@/components/VoiceChat').then(m => m.VoiceChat),
	{ ssr: false },
);
const PremiumPaywallDrawer = dynamic(() =>
	import('@/components/PremiumPaywallDrawer').then(m => m.PremiumPaywallDrawer),
);
...
const starterPrompts = [
	...
	{
		label: 'Action steps',
		prompt: 'Give me practical action steps I can apply today.',
	},
	{
		label: 'Key lessons',
		prompt: 'What are the most important lessons from this book?',
	},
	{
		label: 'Who benefits',
		prompt: 'Who would benefit most from reading this book?',
	},
];
...
export default function ChatDetailPage() {
	...
	const [currentPlan, setCurrentPlan] = useState<
		'free' | 'premium' | 'trial' | null
	>(null);
	const [apiRateLimitedUntil, setApiRateLimitedUntil] = useState<Date | null>(
		null,
	);
	...
	const bookYear =
		typeof bookMeta?.year === 'number' || typeof bookMeta?.year === 'string'
			? String(bookMeta.year)
			: null;
	const firstTopic =
		Array.isArray(bookMeta?.topics) && bookMeta.topics.length > 0
			? bookMeta.topics[0]
			: null;
		...
		(async () => {
			try {
				...
				if (subscriptionRes?.messageInfo) {
					const { remainingMessages: rem, plan: p } =
						subscriptionRes.messageInfo;
					if (typeof rem === 'number' && !Number.isNaN(rem))
						setRemainingMessages(rem);
					if (p === 'free' || p === 'premium' || p === 'trial')
						setCurrentPlan(p);
				}
			} catch (error) {
	...
	const getRemainingTime = (t: Date | null) =>
		t ? Math.max(0, Math.ceil((t.getTime() - Date.now()) / 1000)) : 0;
	const remainingApiSeconds = getRemainingTime(apiRateLimitedUntil);

	useEffect(() => {
		if (remainingApiSeconds > 0) {
			const interval = setInterval(() => {
				if (getRemainingTime(apiRateLimitedUntil) <= 0)
					setApiRateLimitedUntil(null);
			}, 1000);
	...
	const handleSendMessage = useCallback(
		async (content?: string) => {
			...
			try {
				...
				if (
					(Number.isNaN(nextRem) ? 0 : nextRem) === 0 &&
					response.plan === 'free'
				)
				...
				const details =
					error instanceof Error && 'details' in error
						? ((error as Error & { details?: Record<string, unknown> })
								.details ?? null)
						: null;
				...
				if (details?.isApiRateLimit) {
					const waitTime =
						typeof details.waitTime === 'number' && details.waitTime > 0
							? details.waitTime
							: 60;
	...
	const handlePaywallPurchase = async (
		...
		if (data?.messageInfo) {
			...
			if (
				data.messageInfo.plan === 'free' ||
				data.messageInfo.plan === 'premium' ||
				data.messageInfo.plan === 'trial'
			) {
	...
	const handleVoiceConversationComplete = async (
		voiceMessages: { role: 'user' | 'assistant'; content: string }[],
	) => {
		...
		try {
			...
			if (res.ok) {
				...
				setMessages(prev => {
					...
					const dbMessages: Message[] = savedMessages.map(
						(m: {
							id: string;
							content: string;
							role: 'user' | 'assistant';
		...
		return (
			...
			>
				<div className='text-center'>
					...
					<p className='mono text-[11px] text-(--text-muted) uppercase tracking-widest'>
						Opening book...
					</p>
				</div>
			</div>
		...
		>
			...
			<div className='flex-1 flex flex-col min-w-0'>
						...
						>
							...
							<div className='min-w-0'>
								...
								<p className='mono text-[10px] text-(--text-muted) truncate uppercase tracking-widest mt-0.5'>
									{[book?.author, bookYear, firstTopic]
										.filter(Boolean)
										.join(' · ') || 'Unknown'}
								</p>
							</div>
				...
				{activeTab === 'chat' ? (
								...
								{messages.length === 0 ? (
									<div className='flex flex-col items-center justify-center min-h-[50vh] text-center relative'>
										...
										<div className='relative mb-6 z-10'>
											<div className='w-16 h-16 rounded-full ring-2 ring-(--neo-accent)/30 overflow-hidden shadow-[0_0_30px_rgba(201,168,76,0.15)]'>
												{book?.cover_url ? (
													...
													<div className='w-full h-full bg-(--neo-accent-light) flex items-center justify-center'>
														<span className='font-serif text-2xl font-bold text-(--neo-accent)'>
															{bookInitial}
														</span>
													</div>
												)}
										...
										<p className='text-sm text-(--text-muted) max-w-xs mb-8 z-10 leading-relaxed'>
											Ask {book?.title ?? 'this book'} anything — explore ideas,
											themes, and insights.
										</p>
										{/* Starter prompt grid */}
										<div className='grid grid-cols-2 gap-2.5 w-full max-w-sm z-10'>
											{starterPrompts.map(sp => (
												...
												>
													<p className='mono text-[10px] text-(--neo-accent) uppercase tracking-wider mb-1'>
														{sp.label}
													</p>
									...
									messages.map((message, index) => {
										...
										const keyInsight = !isUser
											? extractKeyInsight(message.content)
											: null;
										return (
											<motion.div
												key={message.id}
												...
												{isUser ? (
													...
													<div className='flex gap-3'>
														...
														<div
															...
															style={{
																backgroundColor: 'var(--neo-accent-light)',
															}}
														>
															{book?.cover_url ? (
																<Image
																...
																<span className='mono text-[10px] font-bold text-(--neo-accent)'>
																	{bookInitial}
																</span>
															)}
														</div>
														<div className='flex-1 min-w-0'>
															...
															>
																<ReactMarkdown
																	components={{
																		...
																		strong: ({ children }) => (
																			<strong className='font-semibold text-(--neo-accent)'>
																				{children}
																			</strong>
																		),
																		ol: ({ children }) => (
																		...
																		h1: ({ children }) => (
																			<h3 className='font-serif text-base font-bold text-(--text-primary) mt-4 mb-2'>
																				{children}
																			</h3>
																		),
																		h2: ({ children }) => (
																		...
																		h3: ({ children }) => (
																			<h4 className='font-serif text-sm font-bold text-(--text-primary) mt-3 mb-1.5'>
																				{children}
																			</h4>
																		),
																		code: ({ children }) => (
																			<code className='bg-(--bg-overlay) px-1.5 py-0.5 rounded text-xs mono text-(--text-secondary)'>
																				{children}
																			</code>
																		),
																		blockquote: ({ children }) => (
																			<blockquote className='book-spine pl-4 italic text-(--text-secondary) my-3 font-serif text-[15px]'>
																				{children}
																			</blockquote>
																		),
																	}}
															...
															{keyInsight && (
																<div className='book-spine ink-panel px-4 py-3 mt-2 rounded-md'>
																	...
																	<p className='font-serif text-sm text-(--text-primary) font-medium leading-relaxed italic'>
																		{keyInsight}
																	</p>
																</div>
															)}
					...
					/* Mobile Insights tab */
					<div
						className='flex-1 min-h-0 overflow-y-auto p-5 lg:hidden'
						style={{ backgroundColor: 'var(--chat-bg)' }}
					>
						<h2 className='mono text-[10px] font-medium text-(--neo-accent) uppercase tracking-[0.18em] mb-5'>
							Insights
						</h2>
						<InsightsPanel
				...
				{activeTab === 'chat' && (
					...
					>
							...
							<div className='max-w-3xl mx-auto mb-2 text-center'>
								<p className='text-xs text-(--neo-danger)'>
									...
									<button
										...
										className='text-(--neo-accent) underline cursor-pointer font-medium'
									>
						...
						<div className='max-w-3xl mx-auto flex items-end gap-2'>
								...
								<textarea
									...
									placeholder={
										isBlocked
											...
											: `Ask ${book?.title ?? 'this book'} anything...`
									}
									disabled={isBlocked}
							...
							>
								{remainingApiSeconds > 0 ? (
									<span className='mono text-[10px] font-bold text-(--text-on-accent)'>
										{remainingApiSeconds}
									</span>
								) : (
									<ArrowUp size={15} />
			...
			>
				...
				>
					<p className='mono text-[10px] font-medium text-(--neo-accent) uppercase tracking-[0.18em]'>
						Insights
					</p>
				</div>
			...
			{showShareCard && (
				...
				>
					<div
						...
						className='mx-4 max-h-[80vh] overflow-y-auto'
					>
						<ShareCard
			...
			<PremiumPaywallDrawer
				...
				onPurchase={plan => {
					void handlePaywallPurchase(plan);
				}}
